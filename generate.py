...
JS_APP = r"""
const BUNDLE = { model: { name: RAW_DATA.modelName || "Semantic Model", tables: RAW_DATA.tables }, reports: RAW_DATA.reports, domainMap: RAW_DATA.domains };
const MEASURE_USAGE = {};
...
function highlightDax(e){let s=esc(e);['CALCULATE','FILTER','ALL','ALLEXCEPT','ALLSELECTED','SUM','SUMX','AVERAGE','AVERAGEX','COUNTROWS','COUNT','COUNTA','DISTINCTCOUNT','DIVIDE','IF','SWITCH','VAR','RETURN','BLANK','TRUE','FALSE','RELATED','RELATEDTABLE','SELECTEDVALUE','VALUES','FIRSTDATE','LASTDATE','NOT','ISBLANK','COALESCE','MAX','MAXX','MIN','MINX','EARLIER','RANKX','TOPN','ADDCOLUMNS','SUMMARIZE','KEEPFILTERS','REMOVEFILTERS','USERELATIONSHIP','CROSSFILTER','TREATAS','IN','DATATABLE','ROW','ERROR','FORMAT','YEAR','MONTH','DAY','DATE','TODAY','NOW','DATESYTD','DATESMTD','DATESQTD','TOTALYTD','TOTALMTD','TOTALQTD','SAMEPERIODLASTYEAR','DATEADD','PARALLELPERIOD','PREVIOUSMONTH','PREVIOUSYEAR','NEXTMONTH','NEXTYEAR','STARTOFMONTH','ENDOFMONTH','STARTOFYEAR','ENDOFYEAR','CALENDAR','CALENDARAUTO'].forEach(k=>{s=s.replace(new RegExp('\\b('+k+')\\b','gi'),'<span class="dax-keyword">$1</span>')});return s.replace(/\b(\d+\.?\d*)\b/g,'<span class="dax-number">$1</span>')}
...
function showReport(name){
  ...
  let rows='';
  if(am.length>0){rows+=`<tr class="glossary-cat-header"><td colspan="4"><span class="glossary-cat-icon" style="background:var(--badge-measure)">M</span> Measures (${am.length})</td></tr>`;for(const m of am){const pl=(mp[m]||[]).join(', ');rows+=`<tr class="glossary-row clickable" onclick="showMeasure('${esc(m)}')"><td><span class="glossary-cat-badge" style="background:var(--badge-measure)">Measure</span></td><td class="glossary-term">${esc(m)}</td><td class="glossary-desc"><em style="color:var(--text-muted)">—</em></td><td class="glossary-pages">${esc(pl)}</td></tr>`}}
  rows+=`<tr class="glossary-cat-header"><td colspan="4"><span class="glossary-cat-icon" style="background:var(--badge-report)">P</span> Pages (${pages.length})</td></tr>`;
...
function showMeasure(name){
  ...
  document.getElementById('view-measure').innerHTML=`<div class="breadcrumbs"><a onclick="renderHome()">Home</a><span class="separator">/</span><a onclick="showTable('${esc(st)}')">${esc(st)}</a><span class="separator">/</span><span>${esc(name)}</span></div><div class="section-title">${esc(name)}</div><div class="section-subtitle">Table: ${esc(st)}</div><div class="view-toggle"><button class="active" onclick="toggleView('biz','tech',this)">Business</button><button onclick="toggleView('tech','biz',this)">Technical</button></div><div id="mv-biz"><div class="card"><div class="biz-section"><h4>Description</h4><p style="color:var(--text-muted);font-style:italic">No business description available.</p></div>${depHtml}</div>${uh}</div><div id="mv-tech" class="hidden"><div class="card"><div class="biz-section"><h4>DAX Expression</h4><div class="dax-block">${dax}</div></div></div></div>`;
  showView('measure');
}
...
function showTable(name){
  ...
  document.getElementById('view-table').innerHTML=`<div class="breadcrumbs"><a onclick="renderHome()">Home</a><span class="separator">/</span><span>${esc(name)}</span></div><div class="section-title">${esc(name)}</div><div class="section-subtitle">${typeBadge(table.type)} &middot; ${table.columns.length} columns &middot; ${table.measures.length} measures &middot; Used in ${usedIn.length} reports</div><div class="view-toggle"><button class="active" onclick="toggleView('tbl-biz','tbl-tech',this)">Business</button><button onclick="toggleView('tbl-tech','tbl-biz',this)">Technical</button></div><div id="mv-tbl-biz"><div class="card">${bizDesc}</div>${usedHtml}</div><div id="mv-tbl-tech" class="hidden">${colsHtml}${measHtml}</div>`;
  showView('table');
}

// Sidebar
function renderSidebar(){
  let h='<div style="padding:6px 8px;font-size:11px;font-weight:600;text-transform:uppercase;color:var(--text-muted);letter-spacing:0.5px">Domains</div>';
  for(const[domain,repNames]of Object.entries(BUNDLE.domainMap)){const reps=BUNDLE.reports.filter(r=>repNames.includes(r.name));const domainMeasures=new Set(reps.flatMap(r=>(r.pages||[]).flatMap(p=>p.measures||[]))).size;h+=`<div class="tree-group" data-sb-domain="${esc(domain)}"><div class="tree-group-header" onclick="toggleGroup(this);showDomain('${esc(domain)}')"><span class="chevron open">&#9654;</span><span>${esc(domain)}</span><span class="count">${domainMeasures}</span></div><div class="tree-children">${reps.map(r=>{const ms=[...new Set((r.pages||[]).flatMap(p=>p.measures||[]))].sort();const short=r.name.replace(r.domain+' - ','');return`<div class="tree-group" data-sb-report="${esc(r.name)}"><div class="tree-group-header" onclick="toggleGroup(this);event.stopPropagation();showReport('${esc(r.name)}')"><span class="chevron">&#9654;</span><span style="font-weight:500;font-size:12px">${esc(short)}</span><span class="count">${ms.length}</span></div><div class="tree-children" style="display:none">${ms.map(m=>`<div class="tree-item" data-sb-measure="${esc(m)}" onclick="event.stopPropagation();showMeasure('${esc(m)}')" title="${esc(m)}"><span class="search-type-badge" data-type="measure" style="font-size:9px;padding:1px 4px">M</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(m)}</span></div>`).join('')}${ms.length===0?'<div class="tree-item" style="color:var(--text-muted);font-style:italic;cursor:default">No measures</div>':''}</div></div>`}).join('')}</div></div>`}
  ...
  for(const type of['fact','dimension','measure','parameter','other']){const ts=BUNDLE.model.tables.filter(t=>t.type===type);if(!ts.length)continue;const l=type.charAt(0).toUpperCase()+type.slice(1);h+=`<div class="tree-group" data-sb-tgroup="${type}"><div class="tree-group-header" onclick="toggleGroup(this)"><span class="chevron">&#9654;</span>${typeBadge(type)}<span>${l}</span><span class="count">${ts.length}</span></div><div class="tree-children" style="display:none">${ts.map(t=>`<div class="tree-item" data-sb-table="${esc(t.name)}" onclick="event.stopPropagation();showTable('${esc(t.name)}')">${esc(t.name)}</div>`).join('')}</div></div>`}
...
document.getElementById('sidebar-search').addEventListener('input',function(){
  ...
  panel.querySelectorAll('[data-sb-report]').forEach(el=>{const hasVisible=el.querySelector('[data-sb-measure]:not([style*="display: none"])');const nameMatch=el.dataset.sbReport.toLowerCase().includes(q);el.style.display=(hasVisible||nameMatch)?'':'none';if(hasVisible){el.querySelector('.tree-children').style.display=''}});
  ...
  panel.querySelectorAll('[data-sb-tgroup]').forEach(el=>{const hasVisible=el.querySelector('[data-sb-table]:not([style*="display: none"])');el.style.display=hasVisible?'':'none';if(hasVisible){el.querySelector('.tree-children').style.display=''}});
});
...
function showLineage(reportName, pageName) {
  ...
  document.getElementById('view-lineage').innerHTML = `
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:20px">${pageBtns}</div>
