...
function showMeasure(name){
  ...
  document.getElementById('view-measure').innerHTML=`<div class="breadcrumbs"><a onclick="renderHome()">Home</a><span class="separator">/</span><a onclick="showTable('${esc(st)}')">${esc(st)}</a><span class="separator">/</span><span>${esc(name)}</span></div><div class="section-title">${esc(name)}</div><div class="section-subtitle">Table: ${esc(st)}</div><div class="card"><div class="biz-section"><h4>DAX Expression</h4><div class="dax-block">${dax}</div></div></div>${uh}`;
  showView('measure');
}
